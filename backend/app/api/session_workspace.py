...
def mine(db:Session=Depends(get_db),p:Person=Depends(get_current_person)):
 xs=db.scalars(select(ProgramSession).join(SessionParticipant,SessionParticipant.session_id==ProgramSession.id).where(SessionParticipant.person_id==p.id,SessionParticipant.is_active==True).order_by(ProgramSession.starts_at.desc().nullslast(),ProgramSession.name)).all()
...
def handoffs(sid:UUID,db:Session=Depends(get_db),p:Person=Depends(get_current_person)):
 s,sp,a=access(db,p,sid);xs=db.scalars(select(SessionHandoff).where(SessionHandoff.session_id==sid).order_by(SessionHandoff.created_at.desc())).all();return [{'id':str(x.id),'partner_name':x.partner_name,'delivered_at':x.delivered_at,'summary':x.summary,'impact_notes':x.impact_notes,'entered_by':person_name(db,x.entered_by_person_id)} for x in xs]
