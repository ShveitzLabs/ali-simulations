...
def workspace(sid:UUID,db:Session=Depends(get_db),p:Person=Depends(get_current_person)):
 ...
 return {'id':str(s.id),'name':s.name,'overview':s.overview,'status':s.status,'starts_at':s.starts_at,'ends_at':s.ends_at,'is_admin':is_admin,'my_team_id':str(sp.team_id) if sp and sp.team_id else None,'my_role':sp.current_role_key if sp else ('administrator' if is_admin else None),'categories':cats,'teams':[{'id':str(x.id),'name':x.name} for x in teams],'participants':[{'person_id':str(x.person_id),'name':f'{who.first_name} {who.last_name}','team_id':str(x.team_id) if x.team_id else None,'role':x.current_role_key} for x,who in parts]}
